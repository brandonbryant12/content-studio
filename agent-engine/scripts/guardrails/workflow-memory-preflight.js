...
const DEFAULT_MEMORY_PATH = path.join('agent-engine', 'workflow-memory');

const pnpmCommand = process.platform === 'win32' ? 'pnpm.cmd' : 'pnpm';

const trimOutput = (value) => (typeof value === 'string' ? value.trim() : '');
...
const isTsxMissingError = (output) =>
...
export const runWorkflowMemoryPreflight = ({
  ...
  const initialCheck = commandRunner(cwd, pnpmCommand, ['exec', 'tsx', '--version']);
  ...
  const installResult = commandRunner(cwd, pnpmCommand, [
    'install',
    '--frozen-lockfile',
    '--prefer-offline',
  ]);
  ...
  const retryCheck = commandRunner(cwd, pnpmCommand, ['exec', 'tsx', '--version']);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const exitCode = runWorkflowMemoryPreflight(parseArgs(process.argv.slice(2)));
