  ...
  Effect.gen(function* () {
    ...
    if (operation.id === "issue-evaluator") {
      promptLines.push(
        ...
        "- If a `ready-for-dev` issue is missing routing labels, apply the allowed model/thinking pair in this run.",
  ...
  Effect.gen(function* () {
    ...
    for (let runIndex = 1; runIndex <= maxRuns; runIndex += 1) {
      ...
      const plannerPrompt = [
        ...
        "Allowed model labels: model:gpt-5.4, model:gpt-5.3-codex, model:gpt-5.3-codex-spark",
        ...
        "If a selected issue lacks routing labels, default to model:gpt-5.4 and thinking:xhigh.",
