...
function ResearchCallout({
  ...
  config: NonNullable<SourceDetailSource['researchConfig']>;
}) {
...
export function SourceMetadataBar({ source }: { source: SourceDetailSource }) {
  return (
    <div className="doc-meta-bar">
      <span className={getFileBadgeClass(source.source)}>
      ...
      {source.status === SourceStatus.READY && (
        <>
          ...
          <span className="doc-meta-separator" aria-hidden="true" />
          <span className="doc-meta-item">
            {estimateReadTime(source.wordCount)}
          </span>
        </>
      )}
      ...
      <span
        ...
        title={`Created: ${formatDateTime(source.createdAt)}`}
      >
      ...
      {source.updatedAt !== source.createdAt && (
        <span
          ...
          title={`Updated: ${formatDateTime(source.updatedAt)}`}
        >
          (edited)
        </span>
      )}
...
export function SourceOriginCallout({
  source: sourceData,
}: {
  source: SourceDetailSource;
}) {
  if (sourceData.source === 'url' && sourceData.sourceUrl) {
    const domain = extractDomain(sourceData.sourceUrl);
    return (
  ...
  if (sourceData.source === 'research' && sourceData.researchConfig) {
    return <ResearchCallout config={sourceData.researchConfig} />;
  }
