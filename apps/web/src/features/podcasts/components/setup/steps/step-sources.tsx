...
function getDocumentSourceBadge(doc: SourceItem): string {
...
function ExistingDocumentsPanel({
  ...
  documents: readonly SourceItem[] | null;
  loadingDocs: boolean;
  isError: boolean;
  filteredDocuments: readonly SourceItem[];
  selectedIds: string[];
  searchQuery: string;
...
export function StepSources({
  ...
  const handleSourceCreated = useCallback(
    (sourceId: string, onCreated?: () => void) => {
      ...
      onSelectionChange(
        currentSelectedIds.includes(sourceId)
          ? currentSelectedIds
          : [...currentSelectedIds, sourceId],
      );
