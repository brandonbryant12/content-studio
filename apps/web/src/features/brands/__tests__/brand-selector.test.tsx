...
describe('BrandSelector', () => {
  ...
  it('renders with placeholder when no value selected', () => {
    render(
      <BrandSelector value={null} onChange={vi.fn()} brands={mockBrands} />,
    );
  ...
  it('renders with custom placeholder', () => {
    ...
    expect(
      screen.getByRole('option', { name: 'Choose brand' }),
    ).toBeInTheDocument();
  });

  it('renders selected brand name', () => {
    render(
      <BrandSelector value="brand-1" onChange={vi.fn()} brands={mockBrands} />,
    );
  ...
  it('shows brand with description in option text', () => {
    ...
    expect(
      screen.getByRole('option', { name: /TechCorp - A technology company/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('option', {
        name: /EcoFriendly - Sustainable products/i,
      }),
    ).toBeInTheDocument();
  });
  ...
  it('shows brand without description when description is null', () => {
    ...
    expect(
      screen.getByRole('option', { name: 'HealthFirst' }),
    ).toBeInTheDocument();
  });
  ...
  it('calls onChange with null when placeholder selected', () => {
    const onChange = vi.fn();
    render(
      <BrandSelector value="brand-1" onChange={onChange} brands={mockBrands} />,
    );
  ...
  it('renders with only placeholder when brands empty', () => {
    render(<BrandSelector value={null} onChange={vi.fn()} brands={[]} />);
