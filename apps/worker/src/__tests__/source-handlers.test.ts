...
import type { SourceId, JobId, JobStatus } from '@repo/db/schema';
...
const createMockSourceRepo = (options?: {
  ...
  const service: SourceRepoService = {
    ...
    findById: (id) =>
      Effect.succeed(
        createTestSource({ id: id as SourceId, createdBy: ownerId }),
      ),
    ...
    updateStatus: (id, status, errorMessage) =>
      Effect.sync(() => {
        ...
        return createTestSource({ id: id as SourceId, status });
      }),
    updateContent: (id, data) =>
      Effect.sync(() => {
        options?.onUpdateContent?.(id, data);
        return createTestSource({ id: id as SourceId });
      }),
...
describe('handleProcessUrl', () => {
  ...
  describe('success path', () => {
    it('scrapes URL, uploads content, and updates document', async () => {
      ...
      expect(result.sourceId).toBe('doc_test1');
      ...
      expect(contentData.contentKey).toBe('sources/doc_test1/content.txt');
    ...
    it('uploads content to correct storage path', async () => {
      ...
      expect(key).toBe('sources/doc_abc/content.txt');
    ...
    it('includes optional metadata from scraped content', async () => {
      ...
      const layers = Layer.mergeAll(
        MockDbLive,
        createMockSourceRepo({ onUpdateContent: updateContentSpy }),
        createMockUrlScraper({
          fetchAndExtract: () =>
            Effect.succeed({
  ...
  describe('error handling', () => {
    it('marks document as failed when scraping fails', async () => {
      ...
      const layers = Layer.mergeAll(
        MockDbLive,
        createMockSourceRepo({ onUpdateStatus: updateStatusSpy }),
        createMockUrlScraper({
          fetchAndExtract: () =>
            Effect.fail(
              new UrlFetchError({
    ...
    it('still fails with JobProcessingError even if marking failed also errors', async () => {
      ...
      const failingDocRepoLayer = Layer.succeed(SourceRepo, {
        ...
        findById: (id) =>
          Effect.succeed(
            createTestSource({ id: id as SourceId, createdBy: 'user-123' }),
          ),
        findByIdForUser: () => Effect.die('not implemented'),
        list: () => Effect.die('not implemented'),
        update: () => Effect.die('not implemented'),
