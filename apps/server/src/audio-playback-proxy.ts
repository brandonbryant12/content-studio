...
const parseStorageKeyFromFilesystemUrl = (
  audioUrl: URL,
  storageBaseUrl: string,
): string | null => {
  let base: URL;
  try {
    base = new URL(storageBaseUrl);
  } catch {
    return null;
  }
  ...
  const prefix = pathPrefix(base.pathname);
  if (!audioUrl.pathname.startsWith(prefix)) return null;

  const encodedKey = audioUrl.pathname.slice(prefix.length);
  try {
...
const parseStorageKeyFromS3Url = (
  audioUrl: URL,
  storageConfig: Extract<StorageConfig, { provider: 's3' }>,
): string | null => {
  const endpoint =
...
const parseStorageKey = (
  ...
  if (storageConfig.provider === 'filesystem') {
    return parseStorageKeyFromFilesystemUrl(parsed, storageConfig.baseUrl);
  }
