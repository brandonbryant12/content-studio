...
`PR Risk Review` is a utility skill. Record memory under the parent core workflow key (`Feature Delivery`, `Architecture + ADR Guard`, or `Self-Improvement`) using `pnpm workflow-memory:add-entry` per [`software-factory/workflow-memory/README.md`](../../../software-factory/workflow-memory/README.md). Include the event `id` in output.
