...
After each scan cycle, record one event with workflow key `Periodic Scans` using `node scripts/workflow-memory/add-entry.mjs` per `docs/workflow-memory/README.md`. Include the event `id` in output.
