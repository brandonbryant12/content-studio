...
- Cross-facet audit:
  ...
  - agent-run/eval findings classified with `capability:*` and `failure:*` tags from [`docs/workflow-memory/taxonomy.md`](../../../docs/workflow-memory/taxonomy.md)
...
After each scan cycle, record one event with workflow key `Periodic Scans` using `node agentic-harness-framework/scripts/workflow-memory/add-entry.mjs` per [`docs/workflow-memory/README.md`](../../../docs/workflow-memory/README.md).
If the scan includes memory findings, apply `memory-form:*`, `memory-function:*`, and `memory-dynamics:*` tags from [`docs/workflow-memory/taxonomy.md`](../../../docs/workflow-memory/taxonomy.md).
