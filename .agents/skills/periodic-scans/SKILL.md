...
- Cross-facet audit:
  ...
  - agent-run/eval findings classified with `capability:*` and `failure:*` tags from [`software-factory/workflow-memory/taxonomy.md`](../../../software-factory/workflow-memory/taxonomy.md)
...
After each scan cycle, record one event with workflow key `Periodic Scans` using `pnpm workflow-memory:add-entry` per [`software-factory/workflow-memory/README.md`](../../../software-factory/workflow-memory/README.md).
If the scan includes memory findings, apply `memory-form:*`, `memory-function:*`, and `memory-dynamics:*` tags from [`software-factory/workflow-memory/taxonomy.md`](../../../software-factory/workflow-memory/taxonomy.md).
