...
- Read only the docs needed for the change ([`docs/**/*.md`](../../../docs/)); include concrete anchors like `docs/master-spec.md` and `software-factory/workflows/README.md`.
...
Record one event with workflow key `Feature Delivery` using `pnpm workflow-memory:add-entry` per [`software-factory/workflow-memory/README.md`](../../../software-factory/workflow-memory/README.md), then include the event `id` in output.
...
5. Memory note entry summary with event `id` and `workflow` (what was appended to `software-factory/workflow-memory/events/YYYY-MM.jsonl`)
