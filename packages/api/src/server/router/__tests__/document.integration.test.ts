...
const handlers = {
  create: (args: HandlerArgs): Promise<DocumentOutput> =>
    callORPCHandler<DocumentOutput>(
      documentRouter.create as unknown as {
  ...
  list: (args: HandlerArgs): Promise<DocumentOutput[]> =>
    callORPCHandler<DocumentOutput[]>(
      documentRouter.list as unknown as {
        '~orpc': { handler: (args: unknown) => Promise<unknown> };
      },
      args,
    ),
  get: (args: HandlerArgs): Promise<DocumentOutput> =>
    callORPCHandler<DocumentOutput>(
      documentRouter.get as unknown as {
        '~orpc': { handler: (args: unknown) => Promise<unknown> };
      },
      args,
    ),
  getContent: (args: HandlerArgs): Promise<{ content: string }> =>
    callORPCHandler<{ content: string }>(
      documentRouter.getContent as unknown as {
        '~orpc': { handler: (args: unknown) => Promise<unknown> };
      },
      args,
    ),
  upload: (args: HandlerArgs): Promise<DocumentOutput> =>
    callORPCHandler<DocumentOutput>(
      documentRouter.upload as unknown as {
        '~orpc': { handler: (args: unknown) => Promise<unknown> };
      },
      args,
    ),
  update: (args: HandlerArgs): Promise<DocumentOutput> =>
    callORPCHandler<DocumentOutput>(
      documentRouter.update as unknown as {
        '~orpc': { handler: (args: unknown) => Promise<unknown> };
      },
      args,
    ),
  delete: (args: HandlerArgs): Promise<Record<string, never>> =>
    callORPCHandler<Record<string, never>>(
      documentRouter.delete as unknown as {
        '~orpc': { handler: (args: unknown) => Promise<unknown> };
      },
      args,
    ),
...
describe('document router', () => {
  ...
  describe('upload handler', () => {
    ...
    it('returns DOCUMENT_PARSE_ERROR for malformed PDF uploads', async () => {
      const context = createMockContext(runtime, user);
  ...
  describe('get and getContent handlers', () => {
    ...
    it('returns DOCUMENT_NOT_FOUND for missing or non-owned documents', async () => {
      const context = createMockContext(runtime, user);
      const missingId = generateDocumentId();
      ...
      for (const call of missingCalls) {
        await expectHandlerErrorCode(call, 'DOCUMENT_NOT_FOUND');
      }
  ...
  describe('update handler', () => {
    it('updates multiple fields, recalculates word count, and persists changes', async () => {
      ...
      const [persisted] = await ctx.db
        .select()
        .from(documentTable)
        .where(eq(documentTable.id, created.id as DocumentId));
      expect(persisted).toBeDefined();
  ...
  describe('delete handler', () => {
    it('deletes a document and returns DOCUMENT_NOT_FOUND on second delete', async () => {
      const context = createMockContext(runtime, user);
      const created = await handlers.create({
        context,
        input: {
    ...
    it('returns DOCUMENT_NOT_FOUND for non-owned documents', async () => {
