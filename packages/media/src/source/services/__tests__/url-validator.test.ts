...
describe('validateUrl', () => {
  ...
  describe('private IPs rejected', () => {
    ...
    it('rejects IPv6-mapped loopback IPv4 addresses', async () => {
      const result = await Effect.runPromiseExit(
        validateUrl('http://[::ffff:127.0.0.1]/admin'),
      );
      expect(result._tag).toBe('Failure');
      if (result._tag === 'Failure') {
        const error = result.cause._tag === 'Fail' ? result.cause.error : null;
    ...
    it('rejects IPv6-mapped private IPv4 addresses', async () => {
      const result = await Effect.runPromiseExit(
        validateUrl('http://[::ffff:10.0.0.1]/internal'),
      );
      expect(result._tag).toBe('Failure');
      if (result._tag === 'Failure') {
        const error = result.cause._tag === 'Fail' ? result.cause.error : null;
