...
import type { SourceOrigin, JsonValue } from '@repo/db/schema';
import {
  SourceParseError,
  SourceTooLargeError,
...
const parseTxt = (
  data: Buffer,
  fileName: string,
): Effect.Effect<ParsedSource, SourceParseError> =>
  Effect.try({
    try: () => ({
...
export const parseUploadedFile = (
  ...
  ParsedSource,
  SourceTooLargeError | UnsupportedSourceFormat | SourceParseError
> =>
  Effect.gen(function* () {
    ...
    Effect.withSpan('sources.parseUploadedFile', {
 ...
 */
export const parseSourceContent = (
  input: FileUploadInput,
  ...
  Effect.gen(function* () {
    ...
    Effect.withSpan('sources.parseSourceContent', {
      attributes: { 'file.name': input.fileName },
    }),
  );
