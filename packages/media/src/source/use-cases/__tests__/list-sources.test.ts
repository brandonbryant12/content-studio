import { Db } from '@repo/db/effect';
import {
  createTestUser,
  createTestAdmin,
  createTestSource,
  withTestUser,
  resetAllFactories,
} from '@repo/testing';
import { Effect, Layer } from 'effect';
import { describe, it, expect, beforeEach } from 'vitest';
import type { DatabaseInstance } from '@repo/db/client';
import type { SourceListItem } from '@repo/db/schema';
import {
  SourceRepo,
  type SourceRepoService,
  type ListOptions,
} from '../../repos';
import { listSources } from '../list-sources';

// =============================================================================
// Mock SourceRepo
// =============================================================================

interface MockRepoState {
  sources: SourceListItem[];
}

const createMockSourceRepo = (state: MockRepoState): SourceRepoService => ({
  insert: () => Effect.die('Not implemented in mock'),
  findByIdForUser(id, _userId) {
    return this.findById(id);
  },
  findById: () => Effect.die('Not implemented in mock'),
  update: () => Effect.die('Not implemented in mock'),
  delete: () => Effect.die('Not implemented in mock'),
  updateStatus: () => Effect.die('not implemented'),
  updateContent: () => Effect.die('not implemented'),
  findBySourceUrl: () => Effect.die('not implemented'),
  updateResearchConfig: () => Effect.die('not implemented'),
  findOrphanedResearch: () => Effect.die('not implemented'),

  list: (options: ListOptions) =>
    Effect.succeed(
      state.sources
        .filter((doc) =>
          options.createdBy ? doc.createdBy === options.createdBy : true,
        )
        .slice(
          options.offset ?? 0,
          (options.offset ?? 0) + (options.limit ?? 50),
        ),
    ),

  count: (options?: { createdBy?: string }) =>
    Effect.succeed(
      state.sources.filter((doc) =>
        options?.createdBy ? doc.createdBy === options.createdBy : true,
      ).length,
    ),
});

const createMockSourceRepoLayer = (
  state: MockRepoState,
): Layer.Layer<SourceRepo> =>
  Layer.succeed(SourceRepo, createMockSourceRepo(state));

/**
 * Create a mock Db layer that won't actually be used since
 * our mock SourceRepo methods return Effect.succeed directly.
 * This satisfies the type requirement from the use case signature.
 */
const MockDbLayer: Layer.Layer<Db> = Layer.succeed(Db, {
  db: {} as unknown as DatabaseInstance,
});

// =============================================================================
// Tests
// =============================================================================

describe('listSources', () => {
  beforeEach(() => {
    resetAllFactories();
  });

  describe('regular user', () => {
    it('returns paginated sources for the current user', async () => {
      const user = createTestUser();
      const doc1 = createTestSource({ createdBy: user.id });
      const doc2 = createTestSource({ createdBy: user.id });
      const otherUserDoc = createTestSource({ createdBy: 'other-user-id' });

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: [doc1, doc2, otherUserDoc],
      });

      const effect = listSources({}).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(user)(effect));

      expect(result.sources).toHaveLength(2);
      expect(result.sources.map((d) => d.id)).toEqual([doc1.id, doc2.id]);
      expect(result.total).toBe(2);
      expect(result.hasMore).toBe(false);
    });

    it('returns empty list when user has no sources', async () => {
      const user = createTestUser();
      const otherUserDoc = createTestSource({ createdBy: 'other-user-id' });

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: [otherUserDoc],
      });

      const effect = listSources({}).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(user)(effect));

      expect(result.sources).toHaveLength(0);
      expect(result.total).toBe(0);
      expect(result.hasMore).toBe(false);
    });

    it('respects limit and offset parameters', async () => {
      const user = createTestUser();
      const docs = Array.from({ length: 10 }, () =>
        createTestSource({ createdBy: user.id }),
      );

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: docs,
      });

      const effect = listSources({ limit: 3, offset: 2 }).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(user)(effect));

      expect(result.sources).toHaveLength(3);
      expect(result.sources[0]!.id).toBe(docs[2]!.id);
      expect(result.total).toBe(10);
      expect(result.hasMore).toBe(true);
    });

    it('regular user only sees their own sources (ignores userId filter)', async () => {
      const user = createTestUser();
      const otherUser = createTestUser();
      const userDoc = createTestSource({ createdBy: user.id });
      const otherUserDoc = createTestSource({ createdBy: otherUser.id });

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: [userDoc, otherUserDoc],
      });

      // User tries to filter by another user's ID
      const effect = listSources({ userId: otherUser.id }).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(user)(effect));

      // Should only see their own sources, userId filter is ignored
      expect(result.sources).toHaveLength(1);
      expect(result.sources[0]!.id).toBe(userDoc.id);
      expect(result.total).toBe(1);
    });

    it('hasMore is true when there are more sources beyond offset+limit', async () => {
      const user = createTestUser();
      const docs = Array.from({ length: 15 }, () =>
        createTestSource({ createdBy: user.id }),
      );

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: docs,
      });

      const effect = listSources({ limit: 5, offset: 5 }).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(user)(effect));

      expect(result.sources).toHaveLength(5);
      expect(result.total).toBe(15);
      // offset(5) + returned(5) = 10, total = 15, so hasMore = true
      expect(result.hasMore).toBe(true);
    });

    it('hasMore is false when at the end of the list', async () => {
      const user = createTestUser();
      const docs = Array.from({ length: 10 }, () =>
        createTestSource({ createdBy: user.id }),
      );

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: docs,
      });

      const effect = listSources({ limit: 5, offset: 5 }).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(user)(effect));

      expect(result.sources).toHaveLength(5);
      expect(result.total).toBe(10);
      // offset(5) + returned(5) = 10, total = 10, so hasMore = false
      expect(result.hasMore).toBe(false);
    });
  });

  describe('admin user', () => {
    it('admin can list all sources when no userId specified', async () => {
      const admin = createTestAdmin();
      const user1 = createTestUser();
      const user2 = createTestUser();
      const doc1 = createTestSource({ createdBy: user1.id });
      const doc2 = createTestSource({ createdBy: user2.id });
      const adminDoc = createTestSource({ createdBy: admin.id });

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: [doc1, doc2, adminDoc],
      });

      const effect = listSources({}).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(admin)(effect));

      // Admin sees all sources
      expect(result.sources).toHaveLength(3);
      expect(result.total).toBe(3);
    });

    it('admin can filter by specific userId', async () => {
      const admin = createTestAdmin();
      const targetUser = createTestUser();
      const otherUser = createTestUser();
      const targetDoc = createTestSource({ createdBy: targetUser.id });
      const otherDoc = createTestSource({ createdBy: otherUser.id });

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: [targetDoc, otherDoc],
      });

      const effect = listSources({ userId: targetUser.id }).pipe(
        Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)),
      );

      const result = await Effect.runPromise(withTestUser(admin)(effect));

      // Admin can filter to see only target user's sources
      expect(result.sources).toHaveLength(1);
      expect(result.sources[0]!.id).toBe(targetDoc.id);
      expect(result.total).toBe(1);
    });

    it('admin listing respects pagination with userId filter', async () => {
      const admin = createTestAdmin();
      const targetUser = createTestUser();
      const docs = Array.from({ length: 8 }, () =>
        createTestSource({ createdBy: targetUser.id }),
      );
      const otherDocs = Array.from({ length: 5 }, () =>
        createTestSource({ createdBy: 'other-user-id' }),
      );

      const mockRepoLayer = createMockSourceRepoLayer({
        sources: [...docs, ...otherDocs],
      });

      const effect = listSources({
        userId: targetUser.id,
        limit: 3,
        offset: 2,
      }).pipe(Effect.provide(Layer.mergeAll(mockRepoLayer, MockDbLayer)));

      const result = await Effect.runPromise(withTestUser(admin)(effect));

      expect(result.sources).toHaveLength(3);
      expect(result.sources[0]!.id).toBe(docs[2]!.id);
      expect(result.total).toBe(8);
      expect(result.hasMore).toBe(true);
    });
  });
});
