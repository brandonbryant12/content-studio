...
import { getVoiceoverJob } from '../get-job';
...
const createTestJob = (overrides?: Partial<Job>): Job => ({
  id: 'job_voc_test123' as JobId,
  type: 'generate-voiceover',
  status: 'pending' as JobStatus,
  payload: { voiceoverId: 'voc_123', userId: 'user_123' },
  result: null,
  error: null,
  createdBy: 'user_123',
...
describe('getVoiceoverJob', () => {
  ...
  it('returns job by ID', async () => {
    ...
    const result = await Effect.runPromise(
      getVoiceoverJob({ jobId: job.id }).pipe(
  ...
  it('fails with JobNotFoundError when job does not exist', async () => {
    ...
    const result = await Effect.runPromiseExit(
      getVoiceoverJob({ jobId: 'job_nonexistent' }).pipe(
        Effect.provide(createMockQueue(jobs)),
      ),
    );
