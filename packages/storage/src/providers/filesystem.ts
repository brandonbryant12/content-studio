  ...
  upload: (key, data, _contentType) =>
    Effect.gen(function* () {
      ...
      Effect.withSpan('storage.upload', {
        attributes: { 'storage.key': key, 'storage.provider': 'filesystem' },
      }),
    ),
    ...
    Effect.gen(function* () {
      ...
      const exists = yield* Effect.tryPromise({
        try: () =>
          ...
          new StorageError({ message: `Failed to check file: ${key}`, cause }),
      });
      ...
      return yield* Effect.tryPromise({
        try: () => fs.readFile(filePath),
          ...
          new StorageError({ message: `Failed to read file: ${key}`, cause }),
      });
    }).pipe(
  ...
  delete: (key) =>
    Effect.tryPromise({
      try: () => fs.unlink(path.join(config.basePath, key)),
        ...
        new StorageError({ message: `Failed to delete file: ${key}`, cause }),
    }).pipe(
